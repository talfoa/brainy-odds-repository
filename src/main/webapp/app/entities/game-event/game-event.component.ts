...
export class GameEventComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.gameEventService
      ...
      .subscribe(
        (res: HttpResponse<IGameEvent[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInGameEvents(): void {
    this.eventSubscriber = this.eventManager.subscribe('gameEventListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IGameEvent[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/game-event'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
